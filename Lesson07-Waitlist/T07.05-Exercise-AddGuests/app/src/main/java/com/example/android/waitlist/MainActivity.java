...
public class MainActivity extends AppCompatActivity {
    ...
    public void addToWaitlist(View view) {
        ...
        String stringPartySize = mNewPartySizeEditText.getText().toString();
        if (guestName.length() == 0 || stringPartySize.length() == 0) {
            return;
        }
        ...
        int partySize = 1;
        try {
            partySize = Integer.parseInt(stringPartySize);
        } catch(Exception e) {
            e.fillInStackTrace();
            Log.e(TAG, "addToWaitlist: fail to parse partySize");
        }
